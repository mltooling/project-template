...
if args[build_utils.FLAG_MAKE]:
    ...
    if exit_code > 0:
        build_utils.log(
            "Failed to build mkdocs documentation. "
...
if args[build_utils.FLAG_RELEASE]:
    ...
    build_utils.run("mkdocs gh-deploy --clean", exit_on_error=True, timeout=1)

if args[build_utils.FLAG_RUN]:
    build_utils.log("Run docs in development mode (http://localhost:8001):")
    build_utils.run("mkdocs serve --dev-addr 0.0.0.0:8001", exit_on_error=True)
